function TrapBullet(){
  ...
  this.draw = function(ctx,camera){
    if (this.opacity>=1){
      ...
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate + Math.PI / 3) * this.radius * 0.5)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate + Math.PI / 3) * this.radius * 0.5)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate + Math.PI / 3 * 2) * this.radius * 1.5)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate + Math.PI / 3 * 2) * this.radius * 1.5)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate + Math.PI) * this.radius * 0.5)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate + Math.PI) * this.radius * 0.5)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate - Math.PI / 3 * 2) * this.radius * 1.5)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate - Math.PI / 3 * 2) * this.radius * 1.5)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate - Math.PI / 3) * this.radius * 0.5)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate - Math.PI / 3) * this.radius * 0.5)) * camera.z);
...
TrapBullet.prototype = new DynamicObject();
TrapBullet.prototype.constructor = TrapBullet;
