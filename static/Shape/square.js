function Square(){
  ...
  this.animate = function(tick){
    if (this.isDead || this.health<0 || this.time<0){
      ...
      if (this.opacity === 0){
        system.removeObject(this.id,'shape');
        return;
      }
    }
  ...
  this.draw = function(ctx,camera){
    if (this.opacity>=1){
      ...
      ctx.globalAlpha = 1;
      ctx.beginPath();
      ctx.moveTo((this.x - camera.x + (Math.cos(this.imRotate) * this.radius * 1.2)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate) * this.radius * 1.2)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate + Math.PI / 2) * this.radius * 1.2)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate + Math.PI / 2) * this.radius * 1.2)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate + Math.PI) * this.radius * 1.2)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate + Math.PI) * this.radius * 1.2)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate - Math.PI / 2) * this.radius * 1.2)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate - Math.PI / 2) * this.radius * 1.2)) * camera.z);
      ctx.lineTo((this.x - camera.x + (Math.cos(this.imRotate) * this.radius * 1.2)) * camera.z,(this.y - camera.y + (Math.sin(this.imRotate) * this.radius * 1.2)) * camera.z);
      ctx.fill();
      ctx.stroke();
      ctx.closePath();
    }
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(this.canvasPos.x + Math.cos(this.imRotate) * this.radius * 1.2 * camera.z,this.canvasPos.y + Math.sin(this.imRotate) * this.radius * 1.2 * camera.z);
      this.ctx.lineTo(this.canvasPos.x + Math.cos(this.imRotate + Math.PI / 3 * 2) * this.radius * 1.2 * camera.z,this.canvasPos.y + Math.sin(this.imRotate + Math.PI / 3 * 2) * this.radius * 1.2 * camera.z);
      this.ctx.lineTo(this.canvasPos.x + (Math.cos(this.imRotate + Math.PI) * this.radius * 1.2) * camera.z,this.canvasPos.y + (Math.sin(this.imRotate + Math.PI) * this.radius * 1.2) * camera.z);
      this.ctx.lineTo(this.canvasPos.x + Math.cos(this.imRotate - Math.PI / 3 * 2) * this.radius * 1.2 * camera.z,this.canvasPos.y + Math.sin(this.imRotate - Math.PI / 3 * 2) * this.radius * 1.2 * camera.z);
      this.ctx.lineTo(this.canvasPos.x + Math.cos(this.imRotate) * this.radius * 1.2 * camera.z,this.canvasPos.y + Math.sin(this.imRotate) * this.radius * 1.2 * camera.z);
      this.ctx.fill();
      this.ctx.stroke();
      this.ctx.closePath();
