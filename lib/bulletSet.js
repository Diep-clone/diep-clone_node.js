...
exports.moveBullet = function(b,mapSize,p,target){ // 총알의 움직임
  switch(b.type){
    ...
    case 2: // 드론에게는 goTank 와 goEnemy 라는 두가지 상태가 추가됩니다.
      ...
      if (p){
        if (!b.isAi && p.mouse.right){
          dir = Math.atan2(b.y-p.target.y,b.x-p.target.x);
        }
        else if (!b.isAi && p.mouse.left){
          ...
          switch (b.droneAi){
            ...
            if (b.goTank){
              ...
              if (b.goEnemy.isDead || (b.goEnemy && dis>util.randomRange(278.64,300)) || dis>util.randomRange(139.32,150)) b.goEnemy = undefined;
              else dir = Math.atan2(b.goEnemy.y-b.y,b.goEnemy.x-b.x);
            }
            else if ((b.goEnemy && dis<util.randomRange(278.64,300)) || dis<util.randomRange(139.32,150)){ // 플레이어 주변에 있으면
        ...
        if (dir){
          if (p.mouse !== "" || b.goTank || b.goEnemy !== undefined){
