...
exports.gunSet = function(objects,user,objID,io){
  user.isShot = false;
  for (let i=0;i<user.guns.length;i++){
    ...
    if ((user.owner && user.owner.mouse && user.owner.mouse.left) || user.guns[i].autoShot){
        ...
        case "basic":
          ...
          if (user.guns[i].shotTime <= 0 && user.guns[i].shotPTime < user.guns[i].clickTime/((0.6 - 0.04 * bulletOwner.stats[6]) / user.guns[i].coolTime * 1000)){
            ...
            object = {
              ...
              rotate: rotate,
              dx: Math.cos(rotate) * (2.5 + 0.5 * bulletOwner.stats[3]) * user.guns[i].speed,
              dy: Math.sin(rotate) * (2.5 + 0.5 * bulletOwner.stats[3]) * user.guns[i].speed,
      ...
      if (object){
        ...
        objects.push(object);
        io.emit("shot",user.objType,user.id,i);
      }
    }
    else{
