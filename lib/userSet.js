...
exports.healTank = function (t){
  ...
  else if (Date.now()-4000>t.hitTime){
...
exports.setUserLevel = function(user){
  let statList = "0111111111111111111111111111101001001001001001";
  let expList = [
    0,
    4,
    13,
  ...
  switch(user.type){
      ...
      user.event = {
        ...
        killEvent:function(deader){
          if (deader.objType==="shape" && deader.type===0 && deader.team==="ene" && user.variable.droneCount <= 22 + 2 * user.stats[6]){
            user.variable.droneCount++;
            deader.team = user.team;
            deader.exp = 0;
            deader.owner = user;
            deader.hitObject = null;
            deader.health = deader.maxHealth = (8 + 6 * user.stats[4]) * 0.5;
            deader.damage = (7 + 3 * user.stats[5]) * 1.68;
            deader.speed = (0.056 + 0.01 * user.stats[3]) * 0.5;
            deader.isDead = false;
            deader.deadTime = -1;
            let imF = deader.event.deadEvent;
            deader.event = {
              killEvent:user.event.killEvent,
              deadEvent:function(killer){
                imF();
                user.variable.droneCount--;
              }
            ...
            user.guns[0].bullets.push(deader);
            return false;
          }
          else{
            user.exp += deader.exp;
      ...
      user.event = { // 여기 있는 값들은 모두 "함수" 입니다.
        rightDownEvent:function(){
          user.variable.setCamera = {
            x: user.x + Math.cos(user.rotate) * 400,
            y: user.y + Math.sin(user.rotate) * 400
          };
        }, // 우클릭을 시작했을 때의 이벤트
        rightEvent:function(){
          user.owner.camera.x = user.variable.setCamera.x;
          user.owner.camera.y = user.variable.setCamera.y;
        }, // 우클릭을 진행할 때의 이벤트
        rightUpEvent:function(){}, // 우클릭을 끝낼 때의 이벤트
        killEvent:function(deader){ // 오브젝트를 죽였을 때의 이벤트
          user.exp += deader.exp;
