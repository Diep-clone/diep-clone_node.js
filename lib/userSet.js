...
exports.moveUser = function(u,mapSize,p){ // 유저(탱크)의 움직임
  ...
  if (p && p.rotate!=null){
    ...
    u.dx+=Math.cos(p.rotate) * (0.07 * Math.pow(0.985,u.level-1) + (0.007 * u.stats[7]));
    u.dy+=Math.sin(p.rotate) * (0.07 * Math.pow(0.985,u.level-1) + (0.007 * u.stats[7]));
    isMove = true;
  }
...
exports.isDeadPlayer = function(obj,users){ // 죽었는가?
  ...
  if (obj.health <= 0){
    if (util.findIndex(users,obj.id) > -1){
      obj.isDead=true;
      if (obj.objType === "tank") obj.hitObject.exp += obj.exp;
  ...
  switch(user.type){
      ...
      user.guns = [getGun({
        ...
        bulletAi:function(b,mapSize,p,target){
          if (p && p.mouse.right){
            b.rotate = Math.atan2(b.y-p.target.y,b.x-p.target.x);
            let dis = Math.sqrt((p.target.x-b.x)*(p.target.x-b.x)+(p.target.y-b.y)*(p.target.y-b.y));
            if (dis<60){
              b.dx -= Math.cos(b.rotate) * b.speed;
              b.dy -= Math.sin(b.rotate) * b.speed;
            }
